...
while   (   norm(grad'*p_k) > options.tol_grad && ...
            delta_x > options.tol_x            && ...
            delta_f > options.tol_fun          && ...
            n_iter < options.niter_max) 
   
    % step 4: linesearch    
    n_iter                      =       1+n_iter;
    [x_kp1, fx_kp1, ls_iter]    =       linesearch(fnc, grad, x_k, p_k, ...
                                        options.ls_tkmax, options.ls_beta,...
                                        options.ls_c, options.ls_nitermax);
   
    % step 5: Hessian approximation:
    [~,grad_kp1]                =       gradient_num_comp(fnc, x_kp1, ...
                                        options.gradmethod, options.grad_dx);
    y                           =       (grad_kp1 - grad);
    s                           =       (x_kp1 - x_k);

    % Powell trick to enforce the curvature positivity
