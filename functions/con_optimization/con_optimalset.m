function CON_options = con_optimalset
...
%% General options
CON_options.display    	=	'Iter';     % Display iteration output
CON_options.xsequence   =	'off';      % Store sequence of points {xk}
CON_options.tolgrad    	=	1e-6;       % Termination tolerance on the norm
                                        % of the directional derivative
CON_options.tolconstr   =	1e-6;       % Constraint satisfaction tolerance
CON_options.tolx        =	1e-12;      % Termination tolerance on the relative
                                        % change of optimization variables
CON_options.tolfun      =	1e-12;      % Termination tolerance on the relative
                                        % improvement of the cost function
CON_options.nitermax    =	50;         % Termination tolerance on the number of
                                        % iterations
CON_options.outputfcn   =	[];         % Handle for output function

%% Differentiation options
CON_options.gradmethod  =	'FD';       % Method for gradient computation
                                        % FD    =   Forward Differences
                                        % CD    =   Central Differences
                                        % IM    =   Imaginary-part trick
                                        % UP    =   User Provided
CON_options.graddx      =	2^-26;      % Perturbation for gradient computation
                                        % use 2^-26 for FD
                                        % use 2^-17 for CD
                                        
%% Line search options
CON_options.ls_tkmax    =	1;          % Maximum step size
CON_options.ls_beta     =	0.8;        % Beta scaling factor for 
                                        % back-tracking line search
CON_options.ls_c        =	0.1;        % c coefficient factor for 
                                        % back-tracking line search
CON_options.ls_nitermax =	20;         % max. number of back-tracking 
                                        % line search iterations

%% Quasi-Newton method options
CON_options.Hessmethod  =	'BFGS';     % Method for Hessian computation
                                        % Exact =   Exact Newton. In this
                                        ...
                                        % GN    =   Gauss-Newton algorithm
                                        % SD    =   Steepest Descent
CON_options.BFGS_gamma  =	1e-1;       % gamma factor for Powell's trick in
                                        % BFGS algorithm
CON_options.GN_funF     =	[];         % function providing the value of F
                                        % in Gauss-Newton method, assuming
                                        % f(x)=F(x)'*F(x)
CON_options.GN_sigma    =	0;          % coefficient to ensure Hessian is 
                                        % positive definite in Gauss-Newton
                                        % method (H=GradF*GradF'+eye(n)*sigma)
CON_options.QPoptions   =   ...
